export const filterObjectsTodayInClientTime = (objList) => {
    ...
    const filterCurrentDate = objList.filter(obj => {
        ...
        return (clientTime.getFullYear() === todayYear && clientTime.getMonth() === todayMonth && clientTime.getDate() === todayDate);
    });

        return filterCurrentDate.sort((a, b) => {
        if (a.streamUrlList.some((stream) => stream.isLive) === b.streamUrlList.some((stream) => stream.isLive)) {
        ...
        return b.streamUrlList.some((stream) => stream.isLive)- a.streamUrlList.some((stream) => stream.isLive); // isLive === true (1) lên trước isLive === false (0)
    });
    }